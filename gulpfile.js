...
gulp.task('build:easygo:assets', function (){
    gulp.src('src/easygo/**/*.?(png|jpg|gif|js)', option)
        .pipe(gulp.dest(dist))
        .pipe(browserSync.reload({stream: true}));
});

gulp.task('build:easygo:style', function (){
    gulp.src('src/easygo/index.less', option)
        .pipe(less().on('error', function (e){
            console.error(e.message);
            this.emit('end');
        }))
...
gulp.task('build:easygo:html', function (){
    gulp.src('src/easygo/index.html', option)
        .pipe(tap(function (file){
            var dir = path.dirname(file.path);
            var contents = file.contents.toString();
            contents = contents.replace(/<link\s+rel="import"\s+href="(.*)">/gi, function (match, $1){
...
gulp.task('build:easygo', ['build:easygo:assets', 'build:easygo:style', 'build:easygo:html']);

gulp.task('release', ['build:style', 'build:easygo']);

gulp.task('watch', ['release'], function () {
    gulp.watch('src/style/**/*', ['build:style']);
    gulp.watch('src/easygo/index.less', ['build:easygo:style']);
    gulp.watch('src/easygo/**/*.?(png|jpg|gif|js)', ['build:easygo:assets']);
    gulp.watch('src/**/*.html', ['build:easygo:html']);
});
